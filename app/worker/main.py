import os, json, logging, sys, threading, re, time
...
def get_token()->str:
    if not GITHUB_TOKEN_SECRET_ARN:
        raise RuntimeError("Missing GITHUB_TOKEN_SECRET_ARN")
...
_DIFF_MARK_RE = re.compile(r"^(\+|-|@@|diff --git|index [0-9a-f]+\.\.[0-9a-f]+|\\ No newline)", re.I)
